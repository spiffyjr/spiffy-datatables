...
namespace SpiffyDatatables\Config;
    ...
    HandlerInterface
{
    ...
    public function configure(Config\Event $event)
    {
        ...
        if ($builder instanceof Builder\AbstractServiceManager) {
            if (isset($config['spiffy_datatables']['manager'])) {
                $serviceConfig = new ServiceManager\Config($config['spiffy_datatables']['manager']);
                $serviceConfig->configureServiceManager($this->getServiceLocator()->get('SpiffyDatatables\Manager'));
            }
        }
    }
